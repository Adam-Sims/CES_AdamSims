describe('Bug Report Form', () => {
    ...
    it('3. Valid data is accepted and form submits', () => {
        ...
        // Intercept the API POST request and mock a successful response with message
        cy.intercept('POST', '/api/bugs', {
            statusCode: 201,
            body: { msg: 'Bug reported!' }
        }).as('submitBug');
    ...
    it('4. Displays success message on submit', () => {
        ...
        // Intercept POST and respond with success message
        cy.intercept('POST', '/api/bugs', {
            statusCode: 201,
            body: { msg: 'Bug reported!' }
        }).as('submitBug');
    ...
    it('5. Shows validation error message if backend returns error', () => {
        // Intercept POST and respond with validation error for title
        cy.intercept('POST', '/api/bugs', {
            statusCode: 422,
            body: { errors: { title: ['The title field is required.'] } },
        }).as('submitBug');
    ...
    it('B2. Form resets after successful submission', () => {
        ...
        // Intercept POST and respond successfully
        cy.intercept('POST', '/api/bugs', { statusCode: 201 }).as('submitBug');
    ...
    it('B3. Handles slow API response or failure', () => {
        // Intercept POST and simulate a 2-second delay with a 500 error response
        cy.intercept('POST', '/api/bugs', async (req) => {
        ...
        // Verify success message does NOT appear due to error
        cy.contains('Bug reported!').should('not.exist');
    });
});